...
import { Outbox } from './entities/delivery.entity';
...
export class OutboxPoller {
  ...
  async onModuleInit() {
    try {
      ...
      this.logger.log('Delivery Outbox poller connected to RabbitMQ');
    } catch (err) {
      this.logger.error('Failed to connect delivery outbox poller to RabbitMQ', err);
    }
  }
  ...
  async pollAndPublish() {
    ...
    for (const event of events) {
      try {
        ...
        this.logger.log(`Published delivery event ${event.eventType} for delivery/order ${event.aggregateId}`);
      } catch (err) {
        this.logger.error(`Failed to publish delivery outbox event ${event.id}`, err);
      }
    }
  }
}
